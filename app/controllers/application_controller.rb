class ApplicationController < ActionController::Base
  ...
  def not_authorized!
    render text: 'not_authorized', status: :unauthorized
  end
  ...
  def target_resource!(obj)
    current_resource = {} unless current_resource
    current_resource[target_name.to_sym] = obj
  end
  ...
  def define_params
    self.class.send(:define_method, "#{target_name}_params") do
      ...
      params.require(target_name.to_sym).permit(*policy(target_name.to_sym => (params[:action] != 'create' and target_resource) || klass.constantize).permitted_attributes)
    end
  end
